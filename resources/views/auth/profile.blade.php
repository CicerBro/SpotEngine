...
<div class="mx-auto max-w-3xl space-y-6 p-4">
    ...
    <header class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <h1 class="text-2xl font-semibold tracking-tight text-gray-900 dark:text-slate-100">Profile</h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">Update account details, password and API credentials.</p>
    </header>
        ...
        <form method="POST" action="{{ route('user-profile-information.update') }}" class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div>
                ...
                <input id="name" type="text" name="name" value="{{ old('name', $user->name) }}" required
                       class="block w-full rounded-lg border-0 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm ring-1 ring-inset placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-inset dark:bg-slate-800 dark:text-slate-100 dark:placeholder:text-slate-500
    ...
    {{-- Change password --}}
    <section class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <h2 class="text-base font-semibold text-gray-900 dark:text-slate-100">Change password</h2>
        <form method="POST" action="{{ route('user-password.update') }}" class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
            @csrf @method('PUT')

            <div class="md:col-span-2">
                <label for="current_password" class="mb-1 block text-sm font-medium text-gray-700 dark:text-slate-300">Current password</label>
            ...
            <div>
                <label for="password" class="mb-1 block text-sm font-medium text-gray-700 dark:text-slate-300">New password</label>
            ...
            <div>
                <label for="password_confirmation" class="mb-1 block text-sm font-medium text-gray-700 dark:text-slate-300">Confirm new password</label>
    ...
    {{-- API key --}}
    <section class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <h2 class="text-base font-semibold text-gray-900 dark:text-slate-100">API key</h2>
        <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">Use this key in Sonarr, Radarr, or other Newznab clients.</p>
        <div class="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center">
            <code class="min-w-0 flex-1 break-all rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 font-mono text-sm text-gray-800 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200">{{ $user->api_token }}</code>
